...
async def filter_books(publisher:str,category:str):
    ...
    books = await requests.get(f"{BOOK_SERVICE_URL}/filter",params = {'publisher': publisher, 'category': category})
    return books
...
async def borrow_book(id:int,no_of_days:int,firstName:str):
    ...
    if book.inStock:
        await requests.put(f"{BOOK_SERVICE_URL}/borrow/{id}",data= {'id':id,'no_of_days':no_of_days,'firstName':firstName})
